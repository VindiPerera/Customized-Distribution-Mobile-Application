<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-display text-xl font-semibold text-ink leading-tight">Categories</h2>
            <a href="{{ route('categories.create') }}" class="px-4 py-2 bg-accent text-white text-sm rounded-md hover:bg-accent-hover">+ New Category</a>
        </div>
    </x-slot>
                    ...
                    <tbody>
                        @forelse ($categories as $category)
                            <tr class="border-t border-line">
                                ...
                                <td class="px-4 py-3 text-right text-ink-soft">{{ $category->products_count }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('categories.edit', $category) }}" class="text-accent hover:underline">Edit</a>
